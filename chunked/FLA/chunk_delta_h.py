...
from fla_utils import IS_NVIDIA_BLACKWELL, IS_NVIDIA_HOPPER, USE_CUDA_GRAPH, autotune_cache_kwargs, check_shared_mem
...
@triton.autotune(
    configs=[
        ...
        for num_stages in ([4] if IS_NVIDIA_BLACKWELL else [4, 3, 2] if check_shared_mem('ampere') else [2, 1])
...
@triton.autotune(
    configs=[
        triton.Config({'BV': BV}, num_warps=num_warps, num_stages=num_stages)
        for num_warps in [2, 4]
        for num_stages in ([4] if IS_NVIDIA_BLACKWELL else [4, 3, 2] if check_shared_mem('ampere') else [1])
